...
class AbchrTestCsvDownload(BaseHandler):
    """
    ...
    async def get(self):
        ...
        data = self.db['abchrSurveys'].find(
            await self.overwrite_query_params({'userId': self.user_id, 'testId': ObjectId(test_id)})
        ).sort('createdAt', pymongo.DESCENDING)
        # If there is no data here
        if data.count() == 0:
            self.set_error(404, 'There is no enough ABC/HR Test responses')
            return

        # Build file name with test type and datetime
        csv_name = f"abchr_Test-{datetime.now().strftime('%Y%m%d%H%M%S%f')}.csv"
...
# ABC/HR is special test that requires multiple columns for a question
def build_abchr_row(item):
