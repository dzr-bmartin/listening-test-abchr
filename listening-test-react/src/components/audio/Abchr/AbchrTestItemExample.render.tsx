...
export const AbchrTestItemExampleRender = observer(function (props: { example: AudioExampleModel, active?: boolean }) {
...
const marks = [
  ...
  { value: 4, label: 'Perceptible, but not annoying' },
  { value: 5, label: 'Imperceptible' },
];

export const AbchrRatingBar = observer(function (props: { audio: AudioFileModel }) {
  useEffect(() => {
    // Set a default value
    const num = parseInt(props.audio.value);
    if (!num) props.audio.value = '5';
  });

  return <Box ml={2.5} mb={2} mt={2} style={{ height: 200, width: 200 }}>
    <Slider orientation="vertical" aria-labelledby="vertical-slider" min={1} max={5} step={0.1} marks={marks}
